...
### INPUT - radar_data.csv : trajecotry data where each each row is a position of an aircraft [time, track_id, x, y, z].
...
from train.data_preprocess import data_preprocess
...
df = pd.read_csv(args.input_files[0], names=columns, header=None, index_col=False)
...
for traj_id in all_trajs_id:
    ...
    traj = np.zeros((df_traj.t.shape[-1], 4))
    traj[:, 0] = df_traj.t
    traj[:, 1] = df_traj.x
    traj[:, 2] = df_traj.y
    traj[:, 3] = df_traj.z
...
radar_data = data_preprocess(trajs, input_data)
